...
using Sample.Ethereum.Creature.ContractDefinition;

namespace Sample.Ethereum.Creature
{
    public partial class CreatureService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, CreatureDeployment creatureDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<CreatureDeployment>().SendRequestAndWaitForReceiptAsync(creatureDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, CreatureDeployment creatureDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<CreatureDeployment>().SendRequestAsync(creatureDeployment);
        }

        public static async Task<CreatureService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, CreatureDeployment creatureDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, creatureDeployment, cancellationTokenSource);
            return new CreatureService(web3, receipt.ContractAddress);
        }
        ...
        public CreatureService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
            ContractHandler = web3.Eth.GetContractHandler(contractAddress);
        }

        public Task<string> Erc712VersionQueryAsync(Erc712VersionFunction erc712VersionFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<Erc712VersionFunction, string>(erc712VersionFunction, blockParameter);
        }

        
        public Task<string> Erc712VersionQueryAsync(BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<Erc712VersionFunction, string>(null, blockParameter);
        }
        ...
        public Task<string> BaseTokenURIQueryAsync(BaseTokenURIFunction baseTokenURIFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<BaseTokenURIFunction, string>(baseTokenURIFunction, blockParameter);
        }

        
        public Task<string> BaseTokenURIQueryAsync(BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<BaseTokenURIFunction, string>(null, blockParameter);
        }
        ...
        public Task<string> ExecuteMetaTransactionRequestAsync(ExecuteMetaTransactionFunction executeMetaTransactionFunction)
        {
             return ContractHandler.SendRequestAsync(executeMetaTransactionFunction);
        }

        public Task<TransactionReceipt> ExecuteMetaTransactionRequestAndWaitForReceiptAsync(ExecuteMetaTransactionFunction executeMetaTransactionFunction, CancellationTokenSource cancellationToken = null)
        {
             return ContractHandler.SendRequestAndWaitForReceiptAsync(executeMetaTransactionFunction, cancellationToken);
        }

        public Task<string> ExecuteMetaTransactionRequestAsync(string userAddress, byte[] functionSignature, byte[] sigR, byte[] sigS, byte sigV)
        {
            var executeMetaTransactionFunction = new ExecuteMetaTransactionFunction();
                executeMetaTransactionFunction.UserAddress = userAddress;
                executeMetaTransactionFunction.FunctionSignature = functionSignature;
                executeMetaTransactionFunction.SigR = sigR;
                executeMetaTransactionFunction.SigS = sigS;
                executeMetaTransactionFunction.SigV = sigV;
            
             return ContractHandler.SendRequestAsync(executeMetaTransactionFunction);
        }

        public Task<TransactionReceipt> ExecuteMetaTransactionRequestAndWaitForReceiptAsync(string userAddress, byte[] functionSignature, byte[] sigR, byte[] sigS, byte sigV, CancellationTokenSource cancellationToken = null)
        {
            var executeMetaTransactionFunction = new ExecuteMetaTransactionFunction();
                executeMetaTransactionFunction.UserAddress = userAddress;
                executeMetaTransactionFunction.FunctionSignature = functionSignature;
        ...
        public Task<byte[]> GetDomainSeperatorQueryAsync(GetDomainSeperatorFunction getDomainSeperatorFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<GetDomainSeperatorFunction, byte[]>(getDomainSeperatorFunction, blockParameter);
        }

        
        public Task<byte[]> GetDomainSeperatorQueryAsync(BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<GetDomainSeperatorFunction, byte[]>(null, blockParameter);
        }
        ...
        public Task<BigInteger> GetNonceQueryAsync(string user, BlockParameter blockParameter = null)
        {
        ...
        public Task<string> MintToRequestAsync(string to)
        {
        ...
        public Task<TransactionReceipt> MintToRequestAndWaitForReceiptAsync(string to, CancellationTokenSource cancellationToken = null)
        {
            var mintToFunction = new MintToFunction();
                mintToFunction.To = to;
